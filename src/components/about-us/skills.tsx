          ...
          <div className="grid gap-4 mt-6">
            {Object.keys(skills).map((skill) => (
              <div key={skill} className="flex items-center">
                <div className="rounded-full z-10  bg-inkredible-black text-white border-2 w-10 h-10 flex justify-center items-center text-sm border-orange-600">
                  {skills[skill]}%
                </div>
                <div className="w-full relative rounded-md overflow-hidden -ml-2 h-4 bg-gray-200">
                  <span
                    className={`absolute inset-0 object-cover bg-orange-600 transition-all duration-1000 ease-out ${
                      visible ? `w-[${skills[skill]}%]` : "w-0"
                    }`}
                  ></span>
                </div>
              </div>
