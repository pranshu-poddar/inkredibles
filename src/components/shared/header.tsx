  ...
  const Header = () => {
    ...
    const accountData = { role: memoizedUser?.user.role || "", phone: memoizedUser?.user.phone || "", name: memoizedUser?.user.firstName || "" }
    ...
    Cookies.set('at', user?.id || "", {
      ...
      httpOnly: process.env.NODE_ENV==="development"?false:true, // Helps protect against XSS attacks
