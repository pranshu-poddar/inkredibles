...
const SearchBar = () => {
    ...
    return (
        <div className=" bg-gray-100 relative rounded-full max-w-xl flex items-center w-full shadow-sm border border-gray-200">
            <small onClick={()=>setshowCategories(!showCategories)} className='flex border-r cursor-pointer h-full px-4 items-center min-w-max gap-2'>All Categories{showCategories ? <IoIosArrowUp /> : <IoIosArrowDown />}</small>
                {showCategories && <ul className='absolute left-2 h-32 overflow-y-auto overflow-x-hidden w-max border rounded-md bg-white top-[110%] z-50'>
                    {
                        Category.map((category) => {
                            return <li className='py-2 px-4 min-w-max hover:bg-gray-100' key={category} value={category}>{category}</li>
                        })
                    }
                </ul>}
